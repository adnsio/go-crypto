...
func TestGenerateKey(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, _, err := ecdh25519.GenerateKey(tt.args.rand)
...
func TestGenerateShared(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := ecdh25519.GenerateShared(tt.args.privateKey, tt.args.publicKey)
			if (err != nil) != tt.wantErr {
...
func BenchmarkGenerateKey(b *testing.B) {
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		publicKey, privateKey, err := ecdh25519.GenerateKey(rand.Reader)
		if err != nil {
			b.Fatal(err)
		}
...
func ExampleGenerateKey() {
	alicePublicKey, alicePrivateKey, err := ecdh25519.GenerateKey(rand.Reader)
	if err != nil {
		panic(err)
	}

	bobPublicKey, bobPrivateKey, err := ecdh25519.GenerateKey(rand.Reader)
	if err != nil {
		panic(err)
	}

	aliceShared, err := ecdh25519.GenerateShared(alicePrivateKey, bobPublicKey)
	if err != nil {
		panic(err)
	}

	bobShared, err := ecdh25519.GenerateShared(bobPrivateKey, alicePublicKey)
	if err != nil {
		panic(err)
	}
